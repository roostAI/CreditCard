...
# Test generated by RoostGPT for test weather-api-spec using AI Type Open AI and AI Model gpt-4o
# 
...
Feature: Realtime Weather API
  ...
  Scenario Outline: Get current weather information with valid parameters
    ...
    And match response.current.air_quality.us_epa_index == '#number'
    And match response.current.air_quality.gb_defra_index == '#number'
  ...
  Scenario: Get current weather information with exceeded API key quota
    Given path 'v1/current.json'
    And param key = authToken
    And param q = 'New York'
    When method GET
