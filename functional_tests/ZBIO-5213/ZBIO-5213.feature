...
@positive @payment-plan-proposals
Scenario Outline: Payment plan proposals for overdue balances
  Given a cardholder has a credit card ending in "<last_four_digits>"
  And the account has an overdue balance of <overdue_amount>
  And the cardholder is unable to pay the full balance at once
  When the payment plan proposal system triggers
  Then a structured payment plan proposal should be generated
  And the proposal should display the masked card number "****<last_four_digits>"
  And the proposal should include the total overdue amount of <overdue_amount>
  ...
  And the proposal should include monthly payment amounts
  And the proposal should include payment deadlines
  And the proposal should include acceptance instructions

  Examples:
    | last_four_digits | overdue_amount |
    | 1234            | 1500.00        |
    | 5678            | 850.75         |
    | 9012            | 2250.50        |
    | 3456            | 675.25         |

@positive @collection-agency-involvement
Scenario Outline: Collection agency involvement for unresponsive accounts
  Given a cardholder has a credit card ending in "<last_four_digits>"
  And the cardholder has failed to respond to previous notifications
  And the overdue balance is <balance_amount>
  And the account has been delinquent for <months_delinquent> months
  When the collection agency involvement process triggers
  Then the collection agency should be provided with the last 4 digits "<last_four_digits>"
  And the collection agency should receive account identification information
  And the collection agency should receive the overdue balance amount of <balance_amount>
  And the full credit card number should never be shared with the collection agency
  And increased collection efforts should be documented
  And cardholder should be notified of collection agency involvement

  Examples:
    | last_four_digits | balance_amount | months_delinquent |
    | 1234            | 2500.00        | 6                 |
    | 5678            | 1750.25        | 8                 |
    | 9012            | 3200.50        | 12                |
    | 3456            | 950.75         | 4                 |

@positive @legal-action-initiation
Scenario Outline: Legal action initiation for extreme non-payment cases
  Given a cardholder has a credit card ending in "<last_four_digits>"
  And the account is in extreme default status
  And the total amount owed is <total_owed>
  And all previous collection efforts have been exhausted
  When legal action initiation is required
  Then legal documentation should be prepared
  And the legal documents should include the last 4 digits "****<last_four_digits>"
  And the legal documents should include the total amount owed of <total_owed>
  And the legal documents should include account identification information
  And the full credit card number should never be included in legal documentation
  And proper legal procedures should be followed
  And cardholder should be notified of legal action initiation

  Examples:
    | last_four_digits | total_owed |
    | 1234            | 5000.00    |
    | 5678            | 3750.25    |
    | 9012            | 7500.50    |
    | 3456            | 2850.75    |
